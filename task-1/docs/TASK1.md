...
**Дополнительное бизнес-требование**:

* Поддержка возможности оставлять отзывы и оценки на товары

* Сортировка выдачи товаров на основе оставленных отзывов и оценок
...
| Отзыв | Публичный комментарий покупателя, размещенный на странице товара | Автор, анонимность, текст, оценка |
| Оценка | Показатель потребительского удовлетворения, выраженный в числовых значениях | Значение |
...
| Роль | Возможности |
|------|-------------|
| Клиент | Просмотр каталога, поиск, корзина, оформление заказа (выбор службы доставки из разрешённых продавцом), оплата, отслеживание доставки, получение уведомлений, оставление отзыва  и оценки |
...
Маркетплейс для розничной торговли: клиенты ищут товары, оформляют заказы, оплачивают и отслеживают доставку, оставляют оценки и отзывы; продавцы управляют ассортиментом, ценами и анализируют продажи.
...
sequenceDiagram
    ...
    alt Покупатель заказывал товар
        ...
        Web ->> API: Периодическое сохранение черновика отзыва
...
| Аспект | Описание |
|--------|----------|
| Контекст | Нужна архитектура, которая позволит быстро развивать продукт на начальном этапе при обеспечении высокой доступности, удволетворяя требованиям доступности и надежноси|
...
| Аспект | Описание |
|--------|----------|
| Контекст | Необходимо безопасно принимать онлайн-платежи от клиентов |
...
| Аспект | Описание |
|--------|----------|
| Контекст | Нужен быстрый доступ к часто запрашиваемым данным (каталог) и хранение сессий, чтобы соответсвовать требованиям к производительности |
...
#### ADR-005: Аутентификация с несколькими способами

| Аспект | Описание |
|---------|---------|
| Контекст | Система должна обеспечивать безопасность и удобство входа.|
| Решение | Поддержка логин/пароль, OAuth, SMS-код, email. Все токены JWT/сессии хранятся в кэше.|
| Альтернативы | Только пароль или только OAuth.|
| Обоснование | Многообразие методов повышает удобство для пользователей и снижает риск компрометации, обеспечивая безопасность. Использование кэша ускоряет проверку сессий, удволетворяя требованиям производительности.|
| Последствия | (+) Повышенная безопасность, гибкость, быстрое восстановление сессий. (−) Более сложная логика аутентификации и мониторинга.|

#### ADR-006. Асинхронное обновление рейтингов

| Аспект | Описание |
|--------|----------|
| Контекст | Оценки добавляются и изменяются часто |
| Решение | Асинхронное обновление рейтингов |
| Альтернативы | Синхронное обновление, обновление по расписанию |
| Обоснование | Улучшение отклика пользовательских операций |
| Последствия | Согласованность в конечном итоге, рейтинг может обновляться с задержкой |

#### ADR-007. Выделение модуля оценок и отзывов

| Аспект | Описание |
|--------|----------|
| Контекст | Оценки и отзывы имеют собственный жизненный цикл |
| Решение | Реализация отдельного модуля для работы с оценками и отзывами |
| Альтернативы | Хранить оценки и отзывы как свойства товаров |
| Обоснование | Четкое разделение ответсвенности, масштабирование на создание системы рекомендаций |
| Последствия | Увеличения числа независимых модулей |
...
- Клиент может просмотреть все оставленные им оценки и отзывы на товары

### 7.2. Каталог и поиск

- Просмотр каталога по категориям
- Полнотекстовый поиск товаров
- Фильтрация по цене, наличию, характеристикам, оценке
- Карточка товара: описание, фото, цена, наличие, отзывы, оценка

#### 7.2.1 Отзывы и оценки

- По умолчанию используется сортировка на основании количества отзывов, оценок и средней оценки товара
- Средняя оценка, количество покупок должна быть представлена в карточке товара
- Возможность оставления, редактирования и удаления собственной оценки и отзыва на товар после покупки товара
- Возможность оставления анонимного и неанонимного отзывов
- Возможность оценить, пожаловаться и ответить на отзыв другого покупателя
...
- Сортировка товаров по рейтингу не должна превышать 0.1 секунды при средней нагрузке.
...
- Не допустима задержка более 0.01 секунды после оставления оценки на товар
...
- Веб-сайт и мобильное приложение должны обеспечивать доступность для пользователя не менее 99.5% времени в месяц
- В случае технических проблем при работе веб-сайта не допускается потеря черновиков отзывов
...
- Система защищена от основных видов атак
...
## 9. Ограничения, накладывающиеся спроектированной архитектурой

### 9.1. Ограничения, связанные с модульным монолитом (ADR-001)

**Ограничения**:

* Все модули разворачиваются и масштабируются совместно

* Невозможно независимо масштабировать подсистемы с разным нагрузочным профилем

* Отказ одного модуля влияет на всю систему

**Влияние на развитие**:

* При значительном росте нагрузки потребуется либо вертикальное масштабирование, либо рефакторинг в сторону выделения отдельных сервисов

### 9.2. Ограничения, связанные с внешними зависимостями (ADR-002)

**Ограничения**:

* Поведение системы частично от доступности внешних сервисов

* Ограничены возможности кастомизации платёжных сценариев

* Зависимость от вендора

**Влияние на развитие**:

* Дополнительные расходы на комиссию провайдера

* Ограниченность поддерживаемых сценариев

### 9.3. Ограничения, связанные с асинхронным обновлением рейтингов (ADR-005)

**Ограничения**:

* Невозможно гарантировать мгновенное отражение изменений рейтинга в выдаче

* Оценка на товар влияет на выдачу в конечном итоге

**Влияние на развитие**:

* Архитектура не подходит для сценариев, где рейтинг участвует в критичных бизнес-операциях в реальном времени

### 9.4. Ограничения, связанные с выделением модуля отзывов и оценок (ADR-006)

**Ограничения**:

* Межмодульные зависимости требуют строгого соблюдения контрактов

**Влияние на развитие**:

* Изменение логики модуля и моделей модуля отзывов и оценок требует синхронизации между модулями
